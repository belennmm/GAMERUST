// para 3D
 // 1 tile es igual a 1 world unit
pub const UNITS_PER_TILE: f32 = 1.0;    
pub const FLOOR_Y: f32 = 0.0;            
 pub const WALL_HEIGHT: f32 = 1.2; 
    // wall cube altura
 pub const SPRITE_SIZE: f32 = 1.0;        
 pub const CAMERA_EYE_HEIGHT: f32 = 7.0; 

pub const SPRITE_HEIGHT: f32 = 1.0;

 #[inline]
 pub fn uv_pixels_to_f32(px: [f64; 4]) -> [f32; 4] {
...
// constraits para 3D
pub const CAMERA_EYE_Z: f32 = 3.0;       
// -----

#[inline]
pub fn uv_pixels_to_normalized_f32(px: [f64; 4], atlas_w: f32, atlas_h: f32) -> [f32; 4] {
    let [x, y, w, h] = [px[0] as f32, px[1] as f32, px[2] as f32, px[3] as f32];
    [x / atlas_w, y / atlas_h, w / atlas_w, h / atlas_h]
}


// frames de spawn
pub fn spawn_frames_uv_f32(atlas_w: f32, atlas_h: f32) -> Vec<[f32;4]> {
    SPAWN_FRAMES.iter()
        .map(|&px| uv_pixels_to_normalized_f32(px, atlas_w, atlas_h))
        .collect()
}
